...
from config.settings import (
    ...
    MTF_REFRESH_MINUTES, MTF_CANDLE_COUNT, OANDA_ENV, PAIRS
)
...
class MTFBiasEngine:
    ...
    def get_bias(self, pair):
        ...
        if macro == confirm and macro != "neutral":
            return macro
        # H4 macro leads: when H1 is ranging/neutral, defer to the H4 direction.
        # This captures the most reliable setup: strong H4 trend + H1 pullback entry.
        if macro != "neutral" and confirm == "neutral":
            return macro
        return "neutral"
    ...
    def _refresh_all(self):
        for pair in PAIRS:
            try:
                ...
                print(f"[WARN] MTF refresh failed ({pair}): {e}")
